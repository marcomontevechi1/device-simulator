...
DEFAULTS = {"randsum": 0.1, "randmul": 0.1}
...
class Param:
    ...
    def __init__(
        self,
        name: str = None,
        typ: ParamType = ParamType.analog,
        init_val=None,
        randsum: float = DEFAULTS["randsum"],
        randmul: float = DEFAULTS["randmul"],
    ):
        """
    ...
    def initRand(self):
        ...
        """
        if self.randmul == None:
            self.randmul = DEFAULTS["randmul"]
        if self.randsum == None:
            self.randsum = DEFAULTS["randsum"]

    @property
    def value(self):
        if self.type_ in [ParamType.integer, ParamType.analog]:
            self._value = self._value * random.uniform(
                1 - self.randmul, 1 + self.randmul
            ) + random.uniform(-self.randsum, self.randsum)
        return self._value
    ...
    def value(self, val):
        try:
            ...
            raise BadArgType(
                "Bad value type {}. Should be {}.".format(type(val), self.type_)
            )
    ...
    def parseTypeAndVal(self):
        ...
        types_per_val = {
            ParamType.binary: bool,
            ParamType.analog: (float, int),
            ParamType.integer: int,
            ParamType.string: str,
        }
        initial_vals = {
            ParamType.binary: False,
            ParamType.analog: 0,
            ParamType.integer: 0,
            ParamType.string: "",
        }
        ...
        if not isinstance(self._value, types_per_val[self.type_]):
            msg = "Initial value {} not allowed for type {}!".format(
                self._value, self.type_
            )
            raise Exception(msg)
